...
class Inventory:
    ...
    def update_item(self, id: int, item_name: str = None, qty: int = None, item_category: int = None, item_price: int = None, reorder_level: int = None):
        update_fields = {}
    ...
    def add_item(self, item_name: str, qty: int, item_category: str, item_price: int, reorder_level: int):
        if not self.if_exists(item_name): 
            self.sl.insert_one({ "_id": self.assign_sno(), "name": item_name, "category": item_category, "qty": qty, "price": item_price, "rl": reorder_level})
        else:
            print("Item Exists")
    ...
    def fetch_all(self):
        lst = []
        for x in self.sl.find():
            lst.append({"id": x["_id"], "name": x["name"], "category": x["category"], "quantity": x["qty"], "price": x["price"], "reorderLevel": x["rl"]})
        return lst